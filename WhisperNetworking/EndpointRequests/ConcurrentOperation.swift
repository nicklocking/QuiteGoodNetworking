...
/*
 Abstract base class for a concurrent operation to be used in an NSOperationQueue. A concurrent operation is one that
 uses thread concurrency, i.e. it starts a request on one thread and then does something else on another, like an endpoint
 request. Better described as "asynchronous".
 */
